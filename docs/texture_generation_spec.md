...
- 分辨率：16x16（默认）；状态效果图标按 Minecraft 资源约定使用 18x18。
...
- 状态效果贴图目录：src/main/resources/assets/cybercultivator/textures/mob_effect
...
三张正面效果图标统一使用 14x14 近圆形主体与 2 像素透明边距，避免只靠横向或纵向拉伸区分：S-01 为双脑叶与中央闪电，S-02 为圆形虹膜与瞳孔，S-03 为细胞膜内的心脏与心电折线。四张神经过载图标共用深蓝神经外壳和硬像素轮廓，但不得再复用同一图案。所有图标必须保留透明背景，并在 18x18 原始尺寸下可辨识。

| 资源名 | 中心符号 | 主色 | 尺寸 |
|--------|----------|------|------|
| synaptic_overclock | 圆形双脑叶与中央闪电 | 红黄白 | 18x18 |
| visual_enhancement | 圆形虹膜、瞳孔与镜面高光 | 青蓝白 | 18x18 |
| metabolic_boost | 圆形细胞、心脏与心电折线 | 绿橙黄 | 18x18 |
| neural_overload | 双脑叶与中央过载裂纹 | 红橙黄 | 18x18 |
| neural_overload_s01 | 三节点断路神经网络 | 红橙黄 | 18x18 |
| neural_overload_s02 | 大型故障眼与单条斜线 | 青紫 | 18x18 |
| neural_overload_s03 | 破裂代谢细胞与双毒滴 | 黄绿橙 | 18x18 |

---

## 3. 当前优化状态

截至 v1.2.0，已无已知占位贴图。本轮重新绘制 2 张矿石、13 张旧物品贴图和 4 张神经过载状态图标；物品统一为 16×16，状态图标统一为 18×18，全部使用硬边透明和有限色板。其中矿石、矿物与 Curios 饰品又根据原版贴图基线完成第二轮校正。机器方块贴图已在此前完成，不再列为占位。
...
- 状态效果：三张正面效果统一使用 14x14 近圆形主体，分别以双脑叶闪电、圆形虹膜和细胞内心脏作为中心符号；基础神经过载、S01、S02、S03 共用深蓝机械神经外壳，分别以双脑叶裂纹、三节点断路、大型故障眼和破裂代谢细胞为中心符号。七张均为独立 18×18 RGBA PNG，使用硬边透明并控制在 10 色以内。
...
## 4. 交付检查清单（生成后逐项验收）
- [ ] 物品/方块尺寸是否为 16x16，状态效果是否为 18x18
- [ ] 文件名是否与模型引用一致
...
| 文件 | 结构重点 |
|------|----------|
| `gui/bio_incubator.png` | 底部生长进度、位于对应物品槽左侧的 N/P/D 状态条、单排五槽自动注入通道与右侧成熟资源输出 |
