...
> 自 v1.1.6 起，版本发布说明优先维护于 GitHub Releases 与 README 头部；本文件保留历史归档，仅作为补充参考。

## v1.1.6 — Curios 饰品修复 + 机器交互界面

### 新功能
- **四台机器交互 GUI：** 培养槽、基因拼接机、大气冷凝器、血清灌装机均新增可交互界面与实时状态动画
- **JEI 界面复用：** JEI 配方分类直接裁切机器 GUI 工作区，展示加工流程、进度与配方信息
- **Curios 饰品简化：** 单片镜改为纯 Tooltip 解析基因，移除 HUD 依赖

### Bug 修复
- **饰品资源消耗边界：** 生物脉冲腰带和生命支持箱不再把空堆叠识别为可用物品，避免无限免费消耗
- **净水瓶返还：** 腰带消耗净水瓶后正确返还玻璃瓶；背包满时掉落到玩家身边
- **玩家级冷却：** 生命支持箱应急治疗改为原版玩家物品冷却，替换饰品不再绕过冷却
- **死亡状态保护：** 死亡玩家不再触发生命支持箱治疗或消耗资源
- **Curios 查询异常：** 静默吞掉所有 `Throwable` 改为记录一次警告并安全禁用当次效果
- **公共 API 解耦：** `CyberCultivatorAPI.isCuriosLoaded()` 改用 Forge `ModList`，避免 API 包依赖兼容实现
- **“赛博装备”进阶：** 改为获得任意一件 Curios 饰品即可完成，并同步中英文描述

### 变更
- `LifeSupportPackItem`：副作用消减逻辑改为每 10 tick 批次应用，配置说明与实际约 3 倍消退速度保持一致
- `CurioEventHandler`：腰带每 5 秒工作一次，生命支持箱仅在副作用消减周期或应急治疗可触发时查询 Curios，减少空闲 tick 能力查询

---

## v1.1.5 — API 稳定性修复与 GameTest 覆盖

### 变更
- Activity 统一限制为 1..15，缺失或非法值回退到 5
- 仅修改 Activity 时重新计算默认 duration/amplifier；显式字段继续覆盖默认计算
- 成熟事件取消后保留种子、清零进度且不扣资源/不产出
- 灌装机合成配方调整为顶部红石、中心稀土
- 自定义配方旧索引回退为 -1
- 四类机器 action-bar 消息改为中英文可本地化文本

### Bug 修复
- 修复血清灌装机配方、作物成熟取消语义与 Activity 读取/事件覆盖规则
- 恢复旧配方编号兼容
- 补齐公开 API 的 null 安全、不可变快照和 ItemStack 深拷贝边界
- 返回真实效果注册 ID

### 测试
- 新增 5 项可实际执行的 Forge GameTest 及最小结构模板
...
### 变更
- 记录核心生产链稳定性修复并同步中英文更新日志
- 合并后自动创建带更新说明的 annotated tag 和 GitHub Release

### CI
- 新增构建与可选依赖烟雾测试工作流
- 排除 datagen 缓存并稳定 JAR 输出

---

## v1.1.3 — 代码审查修复 + 漏斗交互优化 + API 完善
