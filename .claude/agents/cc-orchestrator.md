...
原因：
- 后台执行时编排器严格遵循工作流程，不会遗漏步骤
- 手动编排容易跳过日志记录和测试调度
- 编排器的 Edit/Bash/Agent 权限在后台可自动批准

编排器不需要 Write 工具——只编辑已有文件（日志、计划、经验库），新文件由子 agent 创建。
...
- ❌ 后台通知只回复"已确认"，不复述内容
- ❌ **在主会话中手动执行编排流程。** 必须调度编排器 agent 在后台执行，不得在主会话中直接承担编排职责。
